...
def main():
    ...
    for competition in different_competitions_folders[1:]:
        ...
        for WT_file, Mutant_file in zip(WT_files, Mutant_files):
            ...
            WT_denoised = helpers.n2vDenoising(WT_normalized, visualize=True)
            Mutant_denoisded = helpers.n2vDenoising(Mutant_normalized, visualize=True)

            # TODO: make a pretrained model for n2v
            # TODO: work in Colab (so that lucas has access too)

            # start here the segmentation with a pretrained model
            # TODO: make a pretrained model with napari
